...
const ManageVehicle = () => {
  ...
  const handleEdit = (vehicle) => {
    setEditData({
      ...vehicle,
      imagePreview: vehicle?.image || Image_default,
    });
    setShowEditModal(true);
  };

  const handleSaveEdit = () => {
    const updatedList = vehicleList.map((item) =>
  ...
  const handleDelete = (vehicle) => {
    if (window.confirm(`Are you sure you want to delete "${vehicle.name}"?`)) {
      const updated = vehicleList.filter((item) => item.id !== vehicle.id);
  ...
  return (
    <div className="container py-3">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold">Manage Vehicles</h4>
        <button className="btn btn-success px-3" onClick={() => setShowModal(true)}>
